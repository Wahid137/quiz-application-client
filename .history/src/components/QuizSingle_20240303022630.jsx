    ...
    mutationFn: async (id) => {
      try {
        const response = await axios.delete(
          `https://quiz-application-server.vercel.app/dashboard/addquiz/${id}`
        );
      ...
      <div className="w-full text-white p-5 m-5 bg-primary max-w-lg mx-auto pt-8 px-8 my-5 rounded-lg shadow-md ">
        <div>
          ...
          <div className="flex justify-end mt-5">
            <Link to={`/dashboard/editquiz/${_id}`}>
              <div className="bg-accent hover:bg-default  w-10 h-10 flex justify-center items-center rounded me-2">
                <FaRegEdit className="w-5 h-5" />
              </div>
            </Link>
            <div
              onClick={() => handleDelete(_id)}
              className="bg-accent hover:bg-default w-10 h-10 flex justify-center items-center rounded"
            >
